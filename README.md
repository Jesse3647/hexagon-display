...
Downloads stay disabled while the model is updating or invalid. If a layout contains disconnected groups, an **Export group** selector lets you download one group or all of them. Both formats space pods **at least 5 mm apart for printing**, while the preview shows the assembled display. Choose **3MF** to preserve separate pod components; split the assembly into objects in your slicer to rearrange or delete pods. Exports contain geometry, without printer or slicer settings.
